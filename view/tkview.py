...
class View():
	...
	def searchAll(self):
		self.userOpt = 3
		netdata = model.NetDataModel()
		listOfUnitHealth = netdata.searchAllUnitHealth()
		self.showAll(listOfUnitHealth)
	...
	def show(self, unSaude):
		if unSaude:
			...
			self.txtOutputResult.insert('1.0',unSaude.nome)
	...
	def showAll(self, listaDeUnidades):
		if listaDeUnidades:
			...
			for un in listaDeUnidades:
				str += un.nome + '\n'
			self.txtOutputResult.delete('1.0',END)
			self.txtOutputResult.insert('1.0',str)
